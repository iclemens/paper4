function output = model_p3(mdl, mde, params)
    cfg.model = 'wgt';
    cfg.mode = mde;
    ...
    % Fit parameters
    if isnan(params)
        params = fit_data(load_data(source_p3));
    end
    
    for sb = 1:8
        output_p3(sb).fit = params(sb, :);
    end
    ...
    function params_opt = fit_data(data)
        ...
        for s = 1:8
            if strcmp(cfg.mode, 'rp')
                [params_opt(s, :), ~, exitflag] = fminsearch( @(p) 1 - r_squared(data.mup(ss_fit, s), funcrp(p, ss_fit, s)), 0.5 * ones(1, n_params));
            elseif strcmp(cfg.mode, '12')
                [params_opt(s, :), ~, exitflag] = fminsearch( @(p) 1 - r_squared(data.mu2(ss_fit, s), func12(p, ss_fit, s)), 0.5 * ones(1, n_params));
            else
                error('Invalid mode.');
            end 
