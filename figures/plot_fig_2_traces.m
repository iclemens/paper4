function plot_fig_2_traces(experiment)
    ...
    for i = 1:numel(gains_modes)
        ...
        if save_result
            if experiment == 1
                outputFile = sprintf('%s/paper3_figure2_%s_%s.eps', global_config.figure_directory_p3, gains_mode, eye_mode);
            else
                outputFile = sprintf('%s/paper4_figure2_%s_%s.eps', global_config.figure_directory_p4, gains_mode, eye_mode);
            end
            fprintf('Writing %s\n', outputFile);
